...
**状态**: ✅ 已实施 (方案一 + 方案二) + Bug修复

## 📌 实施决定

| 方案 | 决定 | 参数 |
|------|------|------|
| 方案一: 增减载响应优化 | ✅ 采用 | 压力预测窗口30秒 |
| 方案二: 运行时间均衡 | ✅ 采用 | 轮换周期4小时 |
| 方案三: 故障恢复自动重入 | ❌ 暂不采用 | - |
...
3. [ ] 故障恢复确认时间: 暂不采用
4. [ ] 哪些故障允许自动恢复: 暂不采用
5. [x] 是否需要创建独立的 linkage_ctrl 模块: **直接修改usart3.c**
...
| 优先级 | Bug | 修复内容 |
|--------|-----|----------|
| 🔴 P0 | `Max_Address` 越界 | 添加边界检查 `if(Max_Address > 0 && Max_Address <= 10)` |
| 🟡 P1 | `Linkage_CalcPriority` 无参数校验 | 添加 `if(address < 1 \|\| address > 10) return 0.0f;` |
| 🟡 P1 | `Linkage_DoRotation` 可能导致供热中断 | 添加 `runningCount < 2` 时不执行轮换 |
| 🟢 P2 | `Linkage_CheckRotation` 时间回绕 | 处理 `currentMinutes < lastRotationMinutes` 情况 |
| 🟢 P2 | 压力边界硬编码 | 新增 `PRESSURE_MIN_LIMIT/PRESSURE_MAX_LIMIT` 宏 |
